...
#adminPanel input[type=text],#adminPanel input[type=number],#adminPanel textarea,#adminPanel select{width:100%;font:inherit;padding:8px;border-radius:8px;border:1px solid #ffffff44;background:#00000033;color:var(--ink)}
...
<label>Max spins per team<input type="number" id="fMax" min="1"></label>
...
<p class="note">Guests only see the active round and no editing tools. Changes you make here are saved in this browser. To publish them, click "Copy all settings" and paste the result over the DEFAULT block in the file (then bump ver).</p>
</aside>
</main>
<script>
(function(){
var DEFAULT={ver:2,pin:"1234",active:0,rounds:[{
...
options:["GWD","TOA","TOB","Nex","CoX","Maggot King","Muspah","Nightmare/PNM"],removeWinner:false,}]}
choices:["100 KC","150 KC","Purple","25m Uniques","2 GM CAs"],removeWinner:false,maxSpins:5}]};
...
var opts=[],angle=0,spinning=false,pending=false,unlocked=false,team="",log=[];
...
var r=R();opts=r.options.slice(0,24);pending=false;spinning=false;team="";log=[];$("team").value="";
...
show(win+" → "+item,"Choice locked in");pending=false;updateStatus();
});
wrap.appendChild(b);
});
...
for(var i=log.length-1;i>=0;i--){var li=document.createElement("li");li.textContent=log[i].win+(log[i].choice?" → "+log[i].choice:"");h.appendChild(li);}
}
...
var t="Team: "+team+"\n"+log.map(function(e,i){return (i+1)+". "+e.win+(e.choice?" → "+e.choice:"");}).join("\n");
...
$("fRemove").checked=!!r.removeWinner;$("fMax").value=r.maxSpins||5;
...
else if(el.type==="number")r[key]=Math.max(1,parseInt(el.value)||1);
else r[key]=el.value;
save();
...
else if(key==="maxSpins"){updateStatus();}
else{theme();draw();}
});
}
function checkAdmin(){
...
[["fMax","maxSpins"],["fName","name"],["fTitle","title"],["fSub","sub"],["fOpts","options"],["fChoices","choices"],["fRemove","removeWinner"],
